...
export function InstagramTab() {
  ...
  async function gerarLegenda() {
    ...
    setGerando(true);
    try {
      const r = await legendaFn({
        data: {
          titulo,
          mensagem: noticia?.mensagem || "",
          fatos: noticia?.fatos || "",
          leu_materia: noticia?.leu_materia ?? undefined,
        },
      });
                ...
                {(noticiasQ.data ?? []).map((n: any) => (
                  <SelectItem key={String(n.id)} value={String(n.id)}>
                    <div className="flex flex-col">
                      ...
                      {n.leu_materia === false && (
                        <span className="mt-0.5 w-fit rounded bg-orange-100 px-1.5 py-0.5 text-[10px] font-medium text-orange-800">
                          matéria não lida
                        </span>
                      )}
                    </div>
