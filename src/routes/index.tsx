    ...
    try {
      await dispararFn({
        data: {
          ...
          agendadoPara: agendarPara ? new Date(agendarPara).toISOString() : null,
        },
      });
      toast.success(agendarPara ? "Enquete agendada!" : "Enquete disparada!");
      setPergunta("");
      setOpcoes(["", ""]);
      setSelecionada("");
      setAgendarPara("");
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Input
            ...
            type="datetime-local"
            value={agendarPara}
            onChange={(ev) => setAgendarPara(ev.target.value)}
          />
          <p className="text-xs text-muted-foreground">
            Vazio dispara na hora. Agendada, sai na verificacao de hora em hora.
          </p>
        </div>
        <Button
          className="w-full"
          ...
          >
            {disparando ? (
              ...
              <>
                <Send className="w-4 h-4 mr-1" /> {agendarPara ? "Agendar enquete" : "Disparar enquete"}
              </>
            )}
          </Button>
        </CardContent>
