...
export const gerarLegendaInstagram = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const prompt = `Crie uma legenda de Instagram para a Devant Soluções Tributárias sobre esta notícia: ${titulo}. Conteúdo: ${mensagem || ""}. Regras: REESCREVA completamente com suas próprias palavras. Use o NOME REAL das coisas, sem metáforas ou apelidos. O leitor é dono de um negócio pequeno, não é contador. A legenda precisa ser ÚTIL — não só informar o que aconteceu, mas explicar o que o empresário PRECISA FAZER agora. Estrutura: comece pelo impacto direto no bolso ou na rotina do empresário (não pelo fato genérico). Depois explique o contexto de forma simples. Termine com uma ação concreta que a pessoa pode tomar hoje. Use analogias do cotidiano quando ajudar. Tom: direto, confiante, como um amigo que entende do assunto e tá te avisando de algo importante. Entre 8 e 12 linhas. Emojis com moderação. Parágrafos curtos. Varie a abertura. NUNCA use hashtags com o nome da marca (nada de #Devant, #DevantSoluções ou similar). As 5 hashtags finais devem ser apenas sobre o TEMA da notícia (ex: #ReformaTributaria #ImpostoDeRenda). Não invente dados. Responda apenas com a legenda.`;
    const raw = (await callGemini(prompt, 0.9)).trim();
