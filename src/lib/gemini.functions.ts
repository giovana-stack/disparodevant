...
export const gerarLegendaInstagram = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const prompt = `Crie uma legenda de Instagram para a Devant Soluções Tributárias sobre esta notícia: ${titulo}. Conteúdo: ${mensagem || ""}. Regras: REESCREVA completamente com suas próprias palavras. Use o NOME REAL das coisas, sem metáforas ou apelidos. O leitor é dono de empresa, não é contador. Entre 5 e 8 linhas NO MÁXIMO — seja conciso, cada frase precisa justificar sua existência. NUNCA repita a mesma ideia com palavras diferentes. Se já disse pra ligar pro contador, não diga de novo. Estrutura: primeiro o impacto direto no empresário (1-2 linhas), depois o contexto simples (1-2 linhas), depois UMA ação concreta (1 linha). NUNCA use analogias forçadas — só use comparação se for realmente óbvia e natural. Tom: direto, confiante, informal mas profissional. Sem palavras formais como 'certifique-se', 'garanta que', 'é fundamental'. Fale como gente fala. Emojis com moderação. Parágrafos curtos. Varie a abertura. NUNCA use hashtags com o nome da marca. As 5 hashtags finais devem ser apenas sobre o TEMA da notícia. Não invente dados. Responda apenas com a legenda.`;
    const raw = (await callGemini(prompt, 0.9)).trim();
