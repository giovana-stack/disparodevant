...
export const gerarLegendaInstagram = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const prompt = `Crie uma legenda de Instagram para a Devant Soluções Tributárias sobre esta notícia: ${titulo}. Conteúdo da notícia: ${mensagem || ""}. Regras: REESCREVA completamente com suas próprias palavras, NUNCA copie trechos. Use o NOME REAL das coisas, não metáforas ou apelidos (diga 'restituição do Imposto de Renda', não 'dinheiro retido no Leão'; diga 'Receita Federal', não 'o Fisco'). A pessoa que lê não é contadora — ela é dona de um negócio pequeno e precisa entender na primeira leitura sem precisar interpretar. Linguagem simples, direta, sem rodeios. Entre 8 e 12 linhas. Pode usar emojis com moderação. Divida em parágrafos curtos. Comece pelo fato principal de forma clara. Explique o que isso significa na prática pro empresário. Varie a abertura, sem fórmulas repetidas. Finalize com exatamente 5 hashtags relevantes. Não invente dados. Responda apenas com a legenda.`;
    const raw = (await callGemini(prompt, 0.9)).trim();
